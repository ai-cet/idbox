...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=f'Generates file format ({"/".join(supported_extensions)}) from given id-box.json configuration.')
    parser.add_argument('filename', nargs='?', default='filename', help='path to svg file')
    parser.add_argument('-t', '--type', type=str, default="svg", choices=supported_extensions, help='Output type')
    parser.add_argument('-o', '--output', type=str, default="", help='Output filename')

    args = parser.parse_args()
    filename_svg = pathlib.Path(args.filename)
    filename_output = f"{filename_svg.stem}.{args.ext}" if not args.output else args.output
